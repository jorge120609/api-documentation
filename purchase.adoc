...
|customAmountSale|boolean|Indicate if the sale or refund was a single custom amount item sale (see type under <<Product>>) and no products from the Product Library was added to the cart. See the section Custom Amount <<CUSTOM_AMOUNT>> below for more information
|===
...
====
Custom amount is a concept of when the item that is added to the cart doesn't come from the Product Library. Custom amount are used in two very different ways: either as an unnamed item in a normal cart, for example it can be a mix of custom amount items and other products, or it can be a single custom amount sale where the only item is a single custom amount item. The latter is refered to as a custom amount sale in which the actual item and their quantity isn't of importance. It is just something unknown sold for a specific amount of money.

The value of the custom amount sales will be based on the given amount while the value of a sale containing an unnamed item will be based on the same principle as for normal products, e.i. item price * quatity. When refunding custom amount sales only an arbitrary amount is needed while refunds of unnamed items or products will be done by selecting items to refund.

The field `customAmountSale` will indicate if a sale was a custom amount sale or not. The `type` field in the products list will indicate if the item was a custom amount or not. For example, a sale with a single custom amount item would have:
...
----
Note that there is only one item in the cart and it is having a quanity of 1 and no product information such as name, productUuid or locationUuid. These sales will be marked as `customAmountSale:true`. With these custom amount sales, it is possible to make partial refund solely based on an arbitrary amount. The refund amount will be the "`unitPrice`"  of the refund:
[source,json]
----
{
  ...
...
----
here, the original custom amount sale had a value of 1000 but the (partial) refund is only for 500. Note thet even though this is an amount based refund, the quantiy in the refund is -1 so that `unitPrice * quantity` still gives the correct value

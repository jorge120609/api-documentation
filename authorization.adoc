...
### Service Description
The Authorization service implements the OAuth 2.0 protocol. It handles merchant authorization of external integrators and issues access tokens used by authorized clients to call the other parts of the Zettle API.
...
### Assertion Grant
For private integrations, you should implement the assertion grant as described below. This will allow you to retrieve an access token using an API key provided to you by the Zettle account owner.

#### Create API key
The Zettle account owner can create an API key from within the https://my.zettle.com/apps/api-keys[Zettle Backoffice]. The API key is provided in the form of a JWT string and serves as the assertion that the integrator will use when calling the assertion grant. *It is of highest importance that this key is being kept secure!*
...
To solve this, we provide two tracking entry points. They both require the developer to register the integration as a normal OAuth application at https://developer.zettle.com.
